...
<head>
    ...
    <title>Inventaris Alat - Gayza Studio</title>
    ...
    <style>
        ...
        .img-alat { width: 60px; height: 60px; object-fit: cover; border-radius: 8px; border: 1px solid #444; }
...
<div class="d-flex">
    ...
    <div class="col-md-10 p-4" style="height: 100vh; overflow-y: auto;">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="fw-bold">Inventaris Alat Musik</h2>
                <p class="text-white-50 mb-0">Kelola stok dan kondisi alat musik sewaan.</p>
            </div>
            <a href="{{ url('/alat/create') }}" class="btn btn-gold shadow-sm">
                    ...
                    <table class="table table-dark-custom table-hover align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="ps-4">
                                    @if($a->foto)
                                        <img src="{{ asset('storage/'.$a->foto) }}" class="img-alat shadow-sm" alt="Foto">
                                    @else
                                        <div class="img-alat d-flex align-items-center justify-content-center bg-dark text-muted">
                                            <i class="fas fa-guitar"></i>
                                        </div>
                                    @endif
                                </td>
                                <td class="fw-bold">{{ $a->nama_alat }}</td>
                                <td>Rp {{ number_format($a->harga_sewa) }}</td>
                                <td>
                                    <span class="badge bg-secondary text-light">{{ $a->stok }} Unit</span>
                                </td>
                                <td>
                                    @if($a->kondisi == 'Baik')
                                        <span class="badge badge-success px-3 rounded-pill">Baik</span>
                                    @elseif($a->kondisi == 'Rusak')
                                        <span class="badge badge-danger px-3 rounded-pill">Rusak</span>
                                    @else
                                        <span class="badge badge-warning px-3 rounded-pill">Perbaikan</span>
                                    @endif
                                </td>
                                <td class="text-end pe-4">
                                    <a href="{{ url('/alat/edit/' . $a->id_alat) }}" class="btn btn-sm btn-outline-warning me-1">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ url('/alat/hapus/' . $a->id_alat) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus alat ini?');">
                                        @csrf
                                        @method('DELETE')
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-5 text-muted">
                                    <i class="fas fa-guitar fa-3x mb-3 opacity-25"></i><br>
